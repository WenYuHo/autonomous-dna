...
class AgentBrowserSession:
    def __init__(self, session_name: str = "autodna-research", timeout: int = 30):
        self.session_name = session_name
        self.timeout = timeout

    def run(self, command: list[str], json_output: bool = True) -> str | dict:
        ...
        try:
            ...
            proc.wait()
            raise RuntimeError(
                f"agent-browser timed out after {self.timeout}s on: {' '.join(command)}"
            ) from exc
...
def slugify_topic(topic: str, max_len: int = 40) -> str:
    ...
    words = [word for word in topic.lower().split() if word not in stop_words]
    if not words:
...
def build_fallback_report(
    ...
    return (
        ...
        f"- Reason: {reason}\n\n"
        "## Constraints\n"
        "- Web research was unavailable or timed out during agent-browser discovery.\n"
        "- This artifact is provisional and is meant to keep the self-improvement loop moving.\n\n"
        "## Working Assumptions\n"
        f"- Topic under investigation: {topic}\n"
        "- Prioritize deterministic fixes that improve reliability, fallback behavior, and test coverage.\n"
        "- Treat web verification as a follow-up refinement, not a bootstrap requirement.\n\n"
        "## Candidate Directions\n"
        "- Add a timeout-safe fallback path instead of failing the entire command.\n"
        "- Preserve a usable artifact even when the preferred research transport is unavailable.\n"
        "- Add tests for timeout, fallback, and downstream task generation behavior.\n\n"
        "## Next Step\n"
        "- Proceed with task generation using this artifact and validate the resulting changes with tests.\n"
    )
    ...
    if isinstance(links_data, list):
        for href in links_data:
            if (
                ...
                and not any(pattern in href for pattern in exclude_patterns)
            ):
                links.append(href)
    return links
...
def run_research(
    ...
    timeout_ms: int,
    retries: int,
    session_name: str = "autodna-research",
    engine: str = "google",
    depth: int = 1,
    benchmark: bool = False,
) -> str:
    del timeout_ms, retries, benchmark
    ...
    try:
        ...
        try:
            ...
            if any(indicator in snapshot_str for indicator in bot_indicators):
                print(f"  [agent-browser] Bot detection triggered on {engine}. Signaling fallback.")
        ...
        while current_depth <= depth and all_sources:
            ...
            for url in list(all_sources):
                ...
                try:
                    ...
                    if text:
                        report_lines.append(str(text).strip()[:8000])
                    if current_depth < depth:
                        next_level.extend(extract_links_from_page(session, exclude)[:3])
                except Exception as exc:
                    report_lines.append(f"_Failed: {exc}_")
                report_lines.append("\n---\n")
        ...
        print(f"  [agent-browser] Research failed: {exc}")
...
def main():
    ...
    print("--- Autonomous DNA: Web Researcher ---")
    res = run_research(
        topic,
        args.max_sources,
        args.allow_domain,
        args.block_domain,
        not args.no_dedupe_host,
        not args.no_dedupe_url,
        args.timeout_ms,
        max(1, args.retries),
        args.session,
        args.engine,
    ...
    if res == "FALLBACK_REQUIRED":
        print("SIGNAL: FALLBACK_REQUIRED")
        res = build_fallback_report(
            topic,
            "agent-browser search failed or timed out",
        )
        used_fallback = True
    ...
    if used_fallback:
        print(f"[fallback] Saved offline research artifact to: {save_path}")
    else:
