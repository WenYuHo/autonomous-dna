...
### Release branch (client-facing)
End users should only receive the `release` branch. It is generated by CI using
`tools/export_release.py` and `release_manifest.json` to exclude internal
agent tooling and docs. See `docs/release.md` for details.

### Self-improvement (automatic)
`autodna epoch` will run `tools/self_improve.py` automatically when
`self_improve.json` is present and enabled. Pass `--no-self-improve` to skip.
